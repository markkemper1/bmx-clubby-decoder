...
function write({ type, fields }) {
  ...
  fields: {
    ...
    const fieldTypes = (0, fields_1.getFieldsMappedByName)(type);
...
function writeFields(fields, fieldTypes) {
    const fieldWriter = (0, FieldWriter_1.FieldWriter)(fieldTypes);
    const result = [];
...
function writeHeader({ length, recordType }) {
    ...
    const recordTypeCodeEntry = Object.entries(recordTypes_1.recordTypes).find(([key, value]) => value == recordType);
    if (!recordTypeCodeEntry)
        throw new Error(`Record type: ${recordType} could not be found in recordTypes`);
